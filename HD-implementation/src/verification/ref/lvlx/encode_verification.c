...
signature_to_bytes(byte_t *enc, const signature_t *sig)
{
    ...
    enc += nbytes;
    encode_digits(enc, sig->mat_B_aux_can_to_B_aux[0][1], nbytes);
    enc += nbytes;
    encode_digits(enc, sig->mat_B_aux_can_to_B_aux[1][0], nbytes);
    enc += nbytes;
    encode_digits(enc, sig->mat_B_aux_can_to_B_aux[1][1], nbytes);
    enc += nbytes;
...
signature_from_bytes(signature_t *sig, const byte_t *enc)
{
    ...
    enc += nbytes;
    decode_digits(sig->mat_B_aux_can_to_B_aux[0][1], enc, nbytes, NWORDS_ORDER);
    enc += nbytes;
    decode_digits(sig->mat_B_aux_can_to_B_aux[1][0], enc, nbytes, NWORDS_ORDER);
    enc += nbytes;
    decode_digits(sig->mat_B_aux_can_to_B_aux[1][1], enc, nbytes, NWORDS_ORDER);
    enc += nbytes;

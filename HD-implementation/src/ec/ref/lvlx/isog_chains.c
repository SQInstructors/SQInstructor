...
void
ec_curve_standard(ec_curve_t *new, ec_isom_t *isom, const ec_curve_t *old)
{
    ...
    unsigned char buf[3 * FP2_ENCODED_BYTES];
    fp2_encode(buf, &t0);
    fp2_encode(buf + FP2_ENCODED_BYTES, &t1);
    fp2_encode(buf + 2 * FP2_ENCODED_BYTES, &t2);
    int min = 0;
    for (int pos = 1; pos <= 2; pos++) {
      for (int i = 0; i < FP2_ENCODED_BYTES; i++) {
        if (buf[min * FP2_ENCODED_BYTES + i] < buf[pos * FP2_ENCODED_BYTES + i])
            break;
        if (buf[min * FP2_ENCODED_BYTES + i] > buf[pos * FP2_ENCODED_BYTES + i]) {
            min = pos;
            break;
    ...
    fp2_copy(&new->A, &E.A);
    fp2_copy(&new->C, &E.C);
    new->is_A24_computed_and_normalized = false;
    ec_normalize_curve_and_A24(new);
}

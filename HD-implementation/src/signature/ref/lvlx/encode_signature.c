...
static byte_t *
encode_basis(byte_t *enc, const ec_basis_t *B)
{
    fp_encode(enc, &B->P.x.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->P.x.im);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->P.z.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->P.z.im);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->Q.x.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->Q.x.im);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->Q.z.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->Q.z.im);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->PmQ.x.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->PmQ.x.im);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->PmQ.z.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &B->PmQ.z.im);
    enc = enc + FP_ENCODED_BYTES;
    return (enc);
}

static const byte_t *
decode_basis(ec_basis_t *B, const byte_t *enc)
{
    fp_decode(&B->P.x.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->P.x.im, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->P.z.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->P.z.im, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->Q.x.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->Q.x.im, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->Q.z.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->Q.z.im, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->PmQ.x.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->PmQ.x.im, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->PmQ.z.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&B->PmQ.z.im, enc);
    enc = enc + FP_ENCODED_BYTES;
    return (enc);
}
...
secret_key_to_bytes(byte_t *enc, const secret_key_t *sk, const public_key_t *pk)
{
    ...
    // action
    enc = ibz_to_bytes(enc, &sk->secret_action[0][0], TORSION_2POWER_BYTES, false);
    enc = ibz_to_bytes(enc, &sk->secret_action[0][1], TORSION_2POWER_BYTES, false);
    enc = ibz_to_bytes(enc, &sk->secret_action[1][0], TORSION_2POWER_BYTES, false);
    enc = ibz_to_bytes(enc, &sk->secret_action[1][1], TORSION_2POWER_BYTES, false);
    // ec_curve_t image_curve;
    enc = ec_curve_to_bytes(enc, &sk->image_curve);
    // ec_basis_t canonical_basis;
    enc = encode_basis(enc, &sk->canonical_basis);
    // ec_basis_t canonical_basis_tor;
    enc = encode_basis(enc, &sk->canonical_basis_tor);
    // ec_isom_t isom;
    fp_encode(enc, &sk->isom.Nx.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &sk->isom.Nx.im);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &sk->isom.Nz.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &sk->isom.Nz.im);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &sk->isom.D.re);
    enc = enc + FP_ENCODED_BYTES;
    fp_encode(enc, &sk->isom.D.im);
    enc = enc + FP_ENCODED_BYTES;
...
secret_key_from_bytes(secret_key_t *sk, public_key_t *pk, const byte_t *enc)
{
    ...
    enc = ibz_from_bytes(&sk->secret_action[0][0], enc, TORSION_2POWER_BYTES, false);
    enc = ibz_from_bytes(&sk->secret_action[0][1], enc, TORSION_2POWER_BYTES, false);
    enc = ibz_from_bytes(&sk->secret_action[1][0], enc, TORSION_2POWER_BYTES, false);
    enc = ibz_from_bytes(&sk->secret_action[1][1], enc, TORSION_2POWER_BYTES, false);

    enc = ec_curve_from_bytes(&sk->image_curve, enc);

    enc = decode_basis(&sk->canonical_basis, enc);
    enc = decode_basis(&sk->canonical_basis_tor, enc);

    fp_decode(&sk->isom.Nx.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&sk->isom.Nx.im, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&sk->isom.Nz.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&sk->isom.Nz.im, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&sk->isom.D.re, enc);
    enc = enc + FP_ENCODED_BYTES;
    fp_decode(&sk->isom.D.im, enc);
    enc = enc + FP_ENCODED_BYTES;

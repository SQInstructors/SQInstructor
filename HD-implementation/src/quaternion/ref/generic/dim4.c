...
void
ibz_vec_4_mod(ibz_vec_4_t *res, const ibz_vec_4_t *vec, const ibz_t *m)
{
    ibz_mod(&((*res)[0]), &((*vec)[0]), m);
    ibz_mod(&((*res)[1]), &((*vec)[1]), m);
    ibz_mod(&((*res)[2]), &((*vec)[2]), m);
    ibz_mod(&((*res)[3]), &((*vec)[3]), m);
}
...
void
ibz_mat_4x4_mod(ibz_mat_4x4_t *res, const ibz_mat_4x4_t *mat, const ibz_t *m)
{
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ibz_mod(&((*res)[i][j]), &((*mat)[i][j]), m);
        }
    }
}

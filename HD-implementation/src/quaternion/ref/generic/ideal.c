...
void
quat_lideal_lideal_mul(quat_left_ideal_t *product,
                       const quat_left_ideal_t *lideal1,
                       const quat_left_ideal_t *lideal2,
                       const quat_alg_t *alg)
{
    quat_lattice_mul(&(product->lattice), &(lideal1->lattice), &(lideal2->lattice), alg);
    product->parent_order = lideal1->parent_order;
    quat_lideal_norm(product);
}
...
void
quat_lideal_equivalent(quat_left_ideal_t *res,
                       const quat_left_ideal_t *orig,
                       const quat_alg_elem_t *factor,
                       const quat_alg_t *alg)
{
    quat_alg_elem_t aux;
    quat_alg_elem_init(&aux);
    assert(quat_lattice_contains(&(aux.coord), &(orig->lattice), factor));
    quat_alg_elem_copy(&aux, factor);
    ibz_mul(&(aux.denom), &(aux.denom), &(orig->norm));
    quat_alg_conj(&aux, &aux);
    quat_lideal_mul(res, orig, &aux, alg);
    quat_alg_elem_finalize(&aux);
}

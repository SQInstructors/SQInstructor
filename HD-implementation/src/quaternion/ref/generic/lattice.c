...
void
quat_lattice_to_standard_basis(quat_alg_elem_t *x, const quat_lattice_t *lattice, const ibz_vec_4_t *coord)
{
    quat_alg_elem_t elem;
    quat_alg_elem_t sum;
    quat_alg_elem_init(&elem);
    quat_alg_elem_init(&sum);
    for (int i = 0; i < 4; i++) {
        quat_alg_elem_copy_ibz(&elem,
                               &(lattice->denom),
                               &(lattice->basis[0][i]),
                               &(lattice->basis[1][i]),
                               &(lattice->basis[2][i]),
                               &(lattice->basis[3][i]));
        quat_alg_elem_mul_by_scalar(&elem, &((*coord)[i]), &elem);
        quat_alg_add(&sum, &sum, &elem);
    }
    quat_alg_elem_copy(x, &sum);
    quat_alg_elem_finalize(&elem);
    quat_alg_elem_finalize(&sum);
}

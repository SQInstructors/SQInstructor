...
int
quat_test_lattice_to_standard_basis(void)
{
    int res = 0;
    quat_alg_elem_t x;
    ibz_vec_4_t coord, cmp;
    ...
    ibz_vec_4_set(&coord, 1, 0, -1, 6);
    quat_lattice_to_standard_basis(&x, &lat, &coord);
    res = res || !quat_lattice_contains(&cmp, &lat, &x);
    ibz_vec_4_sub(&cmp, &cmp, &coord);
    ...
    if (res != 0) {
        printf("Quaternion unit test lattice_to_standard_basis failed\n");
    }
    quat_alg_elem_finalize(&x);
    ibz_vec_4_finalize(&coord);
    ibz_vec_4_finalize(&cmp);
...
quat_test_lattice(void)
{
    ...
    res = res | quat_test_lattice_to_standard_basis();

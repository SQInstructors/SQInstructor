...
// void quat_lideal_lideal_mul(quat_left_ideal_t *product, const quat_left_ideal_t *lideal1, const quat_left_ideal_t
// *lideal2, const quat_alg_t *alg)
int
quat_test_lideal_lideal_mul(void)
{
    int res = 0;
    ibz_t n;
    quat_lattice_t o, ro, roo, rop;
    quat_left_ideal_t lideal, factor, prod;
    quat_alg_elem_t gen;
    quat_alg_t alg;
    ibz_init(&n);
    quat_alg_elem_init(&gen);
    quat_left_ideal_init(&lideal);
    quat_left_ideal_init(&factor);
    quat_left_ideal_init(&prod);
    quat_lattice_init(&o);
    quat_lattice_init(&ro);
    quat_lattice_init(&roo);
    quat_lattice_init(&rop);
    quat_alg_init_set_ui(&alg, 103);
    ibz_set(&n, 25);
    quat_alg_elem_set(&gen, 1, 2, 3, 0, 2);
    quat_lattice_O0_set(&o);
    quat_lideal_create(&lideal, &gen, &n, &o, &alg);
    quat_lideal_right_order(&ro, &lideal, &alg);
    // recreate lideal as left ideal of ro
    quat_alg_elem_set(&gen, 1, 2, -3, 0, 4);
    quat_lattice_to_standard_basis(&gen, &ro, &(gen.coord));
    ibz_set(&n, 11725);
    quat_lideal_create(&factor, &gen, &n, &ro, &alg);
    quat_lideal_lideal_mul(&prod, &lideal, &factor, &alg);
    ibz_copy(&n, &(prod.norm));
    quat_lideal_norm(&prod);
    res = res || (0 != ibz_cmp(&n, &(prod.norm)));
    quat_lideal_right_order(&rop, &prod, &alg);
    quat_lideal_right_order(&roo, &factor, &alg);
    res = res || !quat_lattice_equal(lideal.parent_order, prod.parent_order);
    res = res || !quat_lattice_equal(&roo, &rop);

    if (res != 0) {
        printf("Quaternion unit test lideal_lideal_mul failed\n");
    }
    ibz_finalize(&n);
    quat_alg_elem_finalize(&gen);
    quat_lattice_finalize(&o);
    quat_lattice_finalize(&ro);
    quat_lattice_finalize(&roo);
    quat_lattice_finalize(&rop);
    quat_alg_finalize(&alg);
    quat_left_ideal_finalize(&lideal);
    quat_left_ideal_finalize(&factor);
...
// void quat_lideal_equivalent(quat_left_ideal_t *res, const quat_left_ideal_t *orig, const quat_alg_elem_t *factor,
// const quat_alg_t *alg);
int
quat_test_lideal_equivalent(void)
{
    int res = 0;
    ibz_t n;
    quat_lattice_t o, ro;
    quat_left_ideal_t lideal, equiv;
    quat_alg_elem_t gen, small_equiv, small_lideal;
    ibz_mat_4x4_t red, gram;
    quat_alg_t alg;
    ibz_init(&n);
    quat_alg_elem_init(&gen);
    quat_alg_elem_init(&small_equiv);
    quat_alg_elem_init(&small_lideal);
    ...
    quat_lattice_to_standard_basis(&gen, &ro, &(gen.coord));
    ibz_set(&n, 11725);
    quat_lideal_create(&lideal, &gen, &n, &ro, &alg);

    quat_alg_elem_set(&gen, 1, 2, 9, 6, -2);
    quat_lattice_to_standard_basis(&gen, &lideal.lattice, &(gen.coord));
    quat_lideal_equivalent(&equiv, &lideal, &gen, &alg);
    quat_lideal_reduce_basis(&red, &gram, &lideal, &alg);
    quat_alg_elem_copy_ibz(
        &small_lideal, &(lideal.lattice.denom), &(red[0][0]), &(red[1][0]), &(red[2][0]), &(red[3][0]));
    ibz_mul(&(small_lideal.denom), &(small_lideal.denom), &(lideal.norm));
    quat_alg_conj(&small_lideal, &small_lideal);
    quat_lideal_mul(&lideal, &lideal, &small_lideal, &alg);
    ibz_mat_4x4_zero(&gram);
    ibz_mat_4x4_zero(&red);
    quat_lideal_reduce_basis(&red, &gram, &equiv, &alg);
    quat_alg_elem_copy_ibz(
        &small_equiv, &(equiv.lattice.denom), &(red[0][0]), &(red[1][0]), &(red[2][0]), &(red[3][0]));
    ibz_mul(&(small_equiv.denom), &(small_equiv.denom), &(equiv.norm));
    quat_alg_conj(&small_equiv, &small_equiv);
    quat_lideal_mul(&equiv, &equiv, &small_equiv, &alg);

    res = res || !quat_lideal_equals(&lideal, &equiv, &alg);

    if (res != 0) {
        printf("Quaternion unit test lideal_equivalent failed\n");
    }
    ibz_finalize(&n);
    quat_alg_elem_finalize(&gen);
    quat_alg_elem_finalize(&small_equiv);
    quat_alg_elem_finalize(&small_lideal);
...
quat_test_lideal(void)
{
    ...
    res = res | quat_test_lideal_lideal_mul();
    res = res | quat_test_lideal_conj_without_hnf();
    res = res | quat_test_lideal_equivalent();

...
int
quat_test_dim4_ibz_vec_4_mod(void)
{
    int res = 0;
    ibz_vec_4_t a, b;
    ibz_t m;
    ...
    ibz_vec_4_init(&b);
    ibz_set(&m, 6);
    ibz_vec_4_set(&a, -1, 2, 6, 10);
    ibz_vec_4_mod(&b, &a, &m);
    res = res || !(ibz_cmp_int32(&(b[0]), 5) == 0);
    res = res || !(ibz_cmp_int32(&(b[1]), 2) == 0);
    res = res || !(ibz_cmp_int32(&(b[2]), 0) == 0);
    res = res || !(ibz_cmp_int32(&(b[3]), 4) == 0);
    ibz_vec_4_mod(&a, &a, &m);
    res = res || !(ibz_cmp_int32(&(a[0]), 5) == 0);
    res = res || !(ibz_cmp_int32(&(a[1]), 2) == 0);
    res = res || !(ibz_cmp_int32(&(a[2]), 0) == 0);
    res = res || !(ibz_cmp_int32(&(a[3]), 4) == 0);

    if (res != 0) {
        printf("Quaternion unit test dim4_ibz_vec_4_mod failed\n");
    }
    ibz_finalize(&m);
    ibz_vec_4_finalize(&a);
    ibz_vec_4_finalize(&b);
...
int
quat_test_dim4_ibz_mat_4x4_mod(void)
{
    int res = 0;
    ibz_t m;
    ibz_mat_4x4_t mat, new, cmp;
    ibz_init(&m);
    ibz_set(&m, 10);
    ibz_mat_4x4_init(&new);
    ibz_mat_4x4_init(&cmp);
    ...
    ibz_set(&(cmp[3][2]), 3);
    ibz_set(&(cmp[3][3]), 7);
    ibz_mat_4x4_mod(&new, &mat, &m);
    res = res || !ibz_mat_4x4_equal(&new, &cmp);
    res = res || ibz_mat_4x4_equal(&mat, &cmp);
    ibz_mat_4x4_mod(&mat, &mat, &m);
    res = res || !ibz_mat_4x4_equal(&mat, &cmp);
    if (res != 0) {
        printf("Quaternion unit test dim4_ibz_mat_4x4_copy failed\n");
    }
...
// void ibz_mat_4x4_solve_system_mod(ibz_vec_4_t *res, const ibz_mat_4x4_t *system, const ibz_vec_4_t *target, const
// ibz_t *mod);
int
quat_test_dim4_ibz_mat_4x4_solve_system_mod(void)
{

    int res = 0;
    ibz_mat_4x4_t mat;
    ibz_vec_4_t sol, target, cmp;
    ibz_t mod;
    ibz_init(&mod);
    ibz_vec_4_init(&cmp);
    ibz_vec_4_init(&sol);
    ...
    ibz_vec_4_set(&target, 12, 9, 1, 1);
    ibz_vec_4_set(&cmp, 2, 1, 3, 2);
    ibz_set(&mod, 16);
    ibz_vec_4_set(&(mat[0]), 1, 1, 3, 0);
    ibz_vec_4_set(&(mat[1]), 1, 2, 5, 3);
    ibz_vec_4_set(&(mat[2]), 0, 4, 1, 5);
    ibz_vec_4_set(&(mat[3]), 1, 2, -1, 0);
    // ibz_mat_4x4_eval(&target,&mat,&cmp);
    ibz_mat_4x4_solve_system_mod(&sol, &mat, &target, &mod);
    ibz_vec_4_sub(&cmp, &cmp, &sol);
    res = res || !ibz_vec_4_is_zero(&cmp);
    ibz_mat_4x4_eval(&cmp, &mat, &sol);
    ibz_vec_4_mod(&cmp, &cmp, &mod);
    ibz_vec_4_sub(&cmp, &cmp, &target);
    res = res || !ibz_vec_4_is_zero(&cmp);
    ibz_vec_4_copy(&cmp, &sol);
    ibz_mat_4x4_solve_system_mod(&target, &mat, &target, &mod);
    ibz_vec_4_sub(&cmp, &cmp, &target);
    res = res || !ibz_vec_4_is_zero(&cmp);

    if (res != 0) {
        printf("Quaternion unit test dim4_ibz_mat_4x4_solve_system_mod failed\n");
    }
    ibz_vec_4_finalize(&cmp);
    ibz_vec_4_finalize(&sol);
...
quat_test_dim4(void)
{
    ...
    res = res | quat_test_dim4_ibz_vec_4_mod();
    ...
    res = res | quat_test_dim4_ibz_mat_4x4_mod();
    ...
    res = res | quat_test_dim4_ibz_mat_4x4_solve_system_mod();

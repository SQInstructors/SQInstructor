...
int
quat_test_dim2_ibz_mat_2x2_det(void)
{
    int res = 0;
    ibz_t det, cmp;
    ibz_mat_2x2_t mat;
    ...
    ibz_set(&cmp, 1);
    ibz_mat_2x2_set(&mat, 1, 0, 0, 1);
    ibz_mat_2x2_det(&det, &mat);
    res = res || ibz_cmp(&cmp, &det);

    ibz_set(&cmp, -7);
    ibz_mat_2x2_set(&mat, 2, 3, 1, -2);
    ibz_mat_2x2_det(&det, &mat);
    res = res || ibz_cmp(&cmp, &det);

    ibz_set(&cmp, 3);
    ibz_mat_2x2_set(&mat, 0, 3, -1, 0);
    ibz_mat_2x2_det(&det, &mat);
    res = res || ibz_cmp(&cmp, &det);

    ibz_set(&cmp, 0);
    ibz_mat_2x2_set(&mat, 3, 3, -1, -1);
    ibz_mat_2x2_det(&det, &mat);
    res = res || ibz_cmp(&cmp, &det);

    if (res != 0) {
        printf("Quaternion unit test dim2_ibz_mat_2x2_det failed\n");
    }
...
// const ibz_t *m);
int
quat_test_dim2_ibz_mat_mat_2x2_mul_mod(void)
{
    int res = 0;
    ibz_t m;
    ...
    if (res != 0) {
        printf("Quaternion unit test dim2_ibz_mat_mat_2x2_mul_mod failed\n");
    }
...
quat_test_dim2(void)
{
    ...
    res = res | quat_test_dim2_ibz_mat_2x2_det();
    res = res | quat_test_dim2_ibz_mat_2x2_eval();
    res = res | quat_test_dim2_ibz_mat_mat_2x2_mul_mod();
